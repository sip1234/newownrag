...
`,
    ...
    knowledgeConfiguration: {
      ...
      controlPermissionsTip: '控制權限決定誰可以修改知識庫、文件和配置。',
      viewPermissionsTip: '查看權限決定誰可以查看和檢索知識庫。「所有人」無需加入任何團隊。',
      chunkTokenNumberTip:
 ...
 NER：使用 spaCy NER 和基於規則的關鍵詞提取來抽取實體和關係，無需 LLM 參與提取過程，速度快且資源消耗低`,
    ...
    setting: {
      ...
      promptDisplayDescription:
        '查看系統使用的預設提示詞，預設提示詞僅供查看。',
      presetPromptType: '預設提示詞類型',
      promptContent: '具體提示詞',
      promptContentDescription: '此提示詞由系統統一維護，無法在目前頁面修改。',
      promptSelectionSaved: '預設提示詞選擇已儲存。',
      noPresetPrompts: '暫無可用的預設提示詞。',
      ruleManagement: '規則管理',
      ruleManagementDescription: '對使用者提問和大模型完成生成後的回答執行確定性規則。',
      ruleExecutionNote:
        '使用者提問會在呼叫模型前檢查，模型回答會在生成完成後檢查。拒絕回答會回傳設定內容；引導補充會把設定語句追加到模型回答末尾；直接回答不修改內容，只阻止低優先級規則干預回答。優先級數值越小越先執行。',
      addRule: '新增規則',
      editRule: '編輯規則',
      noRules: '尚未建立規則。',
      ruleEditorDescription:
        '可新增多個關鍵詞與正則條件，任一條件命中即可觸發規則。',
      ruleName: '規則名稱',
      priority: '優先級',
      keywords: '關鍵詞條件',
      keyword: '關鍵詞',
      keywordPlaceholder: '請輸入關鍵詞',
      regexPatterns: '正則表達式條件',
      regexPlaceholder: '請輸入正則表達式',
      ruleAction: '觸發動作',
      ruleResponse: '回答內容',
      enableRule: '啟用規則',
      ruleRequiredFields: '請填寫規則名稱與至少一個條件；拒絕回答和引導補充還需填寫回答內容。',
      ruleActions: {
        reject: '拒絕回答',
        guidance: '引導補充',
        direct_answer: '直接回答（優先級保護）',
      },
      promptTypes: {
        task_analysis: '任務分析',
        plan_generation: '計畫生成',
        reflection: '反思',
        citation_guidelines: '引用規範',
      },
      team: '團隊',
      api: 'API',
      logout: '登出',
