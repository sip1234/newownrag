...
例如：一条带有 1024 维嵌入的 1 KB 消息大约使用 9 KB。5 MB 的默认限制大约可容纳 500 条此类消息。`,
    ...
    knowledgeDetails: {
      metadata: {
        ...
        importFields: '导入字段',
        importFormat:
          'JSON 格式：[{"key":"department","type":"string","description":"…","enum":["财务"]}]。支持 string、list、time、number。',
        importSuccess: '已导入 {{count}} 个元数据字段，请保存后生效。',
        importFailed: '无法导入元数据字段。',
        predefinedField: '预设字段',
        selectPredefinedField: '选择预设字段',
        selectPredefinedValue: '选择预设值',
      ...
      parseFile: '解析文件',
      enableAfterParsing: '请在文档解析完成后再启用。',
      emptyMetadata: '无元数据',
      localUpload: '本地上传',
      fileSize: '文件大小',
      fileType: '文件类型',
...
`,
    ...
    knowledgeConfiguration: {
      ...
      controlPermissionsTip: '控制权限决定谁可以修改知识库、文档和配置。',
      viewPermissionsTip: '查看权限决定谁可以查看和检索知识库。“所有人”无需加入任何团队。',
      chunkTokenNumberTip:
...
示例：general/v2/`,
      ...
      promptDisplayDescription:
        '查看和管理提示词，并将其应用为聊天助手实际使用的系统提示词。',
      presetPromptType: '预设提示词类型',
      promptContent: '具体提示词',
      promptContentDescription:
        '可以直接修改并保存此提示词；应用后将作为所选聊天助手的系统提示词。',
      promptSelectionSaved: '预设提示词选择已保存。',
      noPresetPrompts: '暂无可用的预设提示词。',
      addPrompt: '新增提示词',
      addPromptDescription: '创建自定义提示词并添加到提示词类型列表。',
      promptTypeName: '提示词类型名',
      promptTypeNamePlaceholder: '请输入提示词类型名',
      promptContentPlaceholder: '请输入具体提示词',
      promptRequiredFields: '提示词类型名和具体提示词均为必填项。',
      promptTypeAlreadyExists: '该提示词类型名已存在。',
      promptCreated: '提示词已新增。',
      targetChatAssistant: '目标聊天助手',
      selectChatAssistant: '请选择聊天助手',
      selectChatAssistantRequired: '请先选择要应用提示词的聊天助手。',
      applyToChatAssistant: '应用到聊天助手',
      applyPromptTip:
        '应用后会替换目标聊天助手当前的系统提示词，并从下一次对话开始生效。',
      applyPromptDescription:
        '应用后会替换“{{name}}”当前的系统提示词，并从下一次对话开始生效。',
      savePromptChanges: '保存修改',
      promptContentRequired: '具体提示词不能为空。',
      promptUpdated: '提示词修改已保存。',
      ruleManagement: '规则管理',
      ruleManagementDescription: '对用户提问和大模型完成生成后的回答执行确定性规则。',
      ruleExecutionNote:
        '用户提问会在调用模型前检查，模型回答会在生成完成后检查。拒绝回答会返回配置内容；引导补充会把配置语句追加到模型回答末尾；直接回答不修改内容，仅阻止低优先级规则干预回答。优先级数值越小越先执行。',
      addRule: '新增规则',
      editRule: '编辑规则',
      noRules: '暂未创建规则。',
      ruleEditorDescription:
        '可添加多个关键词和正则条件，任一条件命中即可触发规则。',
      ruleName: '规则名称',
      priority: '优先级',
      keywords: '关键词条件',
      keyword: '关键词',
      keywordPlaceholder: '请输入关键词',
      regexPatterns: '正则表达式条件',
      regexPlaceholder: '请输入正则表达式',
      ruleAction: '触发动作',
      ruleResponse: '回答内容',
      enableRule: '启用规则',
      ruleRequiredFields: '请填写规则名称、至少一个条件；拒绝回答和引导补充还需填写回答内容。',
      ruleActions: {
        reject: '拒绝回答',
        guidance: '引导补充',
        direct_answer: '直接回答（优先级保护）',
      },
      promptTypes: {
        task_analysis: '任务分析',
        plan_generation: '计划生成',
        reflection: '反思',
        citation_guidelines: '引用规范',
      },
      team: '团队',
      system: '系统',
      logout: '登出',
